...
const SiteMapSection = () =>
{
  ...
  return (
    <Box bgcolor={"primary.main"} py={deviceType === "tablet" ? "7.5rem" : "5rem"}>
      <Container>
        <Typography variant={deviceType === "tablet" ? "display3" : "h3"} component={'h2'} fontWeight={900} mb={deviceType === "tablet" ? 15 : 5}>
                ...
                <List>
                  {
                    item.items?.map((item, index) => (
                      <ListItem key={index} disablePadding sx={{
                        ...
                        <ListItemText
                          primary={
                          <NavLink href={item.url} color={'common.black'} onClick={(event) => handleClick(event, item.url.split('#')[1] || '')}>
                            <Typography
                              variant='h4'
                              component={'span'}
                              fontWeight={900}
