...
const SiteMapSection = () =>
{
  ...
  return (
    <Box bgcolor={"primary.main"} py={deviceType === "tablet" ? "7.5rem" : "5rem"}>
      <Container>
        <Typography variant='display3' component={'h2'} fontWeight={900} mb={deviceType === "tablet" ? 15 : 5}>
          無論新創或連鎖餐廳，<br />選擇"PointPro"，<br />助您業務飛躍！
        </Typography>
              ...
              index === 0 ? (
                <Grid item xs={12} md={4} key={index}>
                  <Link href={item.url} onClick={(event) => handleClick(event, 'hero-section')}>
                    <ListItem>
                      <img src="/src/assets/Logo_text.svg" alt="PointPro Logo" />
                    </ListItem>
                  </Link>
                </Grid>
              ) :
                ...
                <List>
                  {
                    item.items?.map((item, index) => (
                      <ListItem key={index} disablePadding sx={{
                        ...
                        <ListItemText
                          primary={
                          <Link href={item.url} underline={'none'} color={'common.black'} onClick={(event) => handleClick(event, item.url.split('#')[1] || '')}>
                            <Typography
                              variant='h4'
                              component={'span'}
                              fontWeight={900}
