...
export const BottomDrawer = () => {
  ...
  return (
    <Drawer open={isShowBottomDrawer} onClose={onClose} anchor="bottom">
      <Box
        component="form"
        ...
        <FormControl sx={{ width: "100%" }}>
          ...
          {currentMeal?.specialities.map((speciality) => (
            <Box key={speciality.id}>
              <FormLabel id={speciality.id}>{speciality.title}:</FormLabel>
              ...
              {speciality.type === "multiple" && (
                <FormGroup row aria-labelledby={speciality.id} id={speciality.id}>
                  {speciality.items.map((item) => (
                    <FormControlLabel
                      key={item.id}
                      value={item.id}
